...
internal object RecentFollowerSelector {
    fun viewportSignature(nodes: List<NodeSnapshot>): String = nodes
        ...
        .mapNotNull { node -> listOfNotNull(node.text, node.contentDescription)
            .firstNotNullOfOrNull(AccountSwitcherInspector::dedicatedHandle)?.let {
                "$it:${node.bounds.top}:${node.bounds.bottom}"
            } }.sorted().joinToString("|")
